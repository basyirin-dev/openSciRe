...
class TestGapAnalyzer:
    def test_full_analysis_no_gaps(self) -> None:
        ...
        items = [
            ...
            _make_ref(ReferenceSource.semantic_scholar, 2026, language="zh", abstract="Clinical trial results"),
        ]
    ...
    def test_subtopics_param(self) -> None:
        analyzer = GapAnalyzer({
            "gap_min_sources": 2, "gap_min_papers": 1,
            "gap_recency_years": 99,
        })
        items_a = [
        ...
        items_b = [
            ...
            _make_ref(ReferenceSource.openalex, year=2026, language="en", abstract="Clinical trial"),
        ]
        report = analyzer.analyze("topic", items_a + items_b, subtopics={"a": items_a, "b": items_b})
        assert len(report.gaps) == 1
