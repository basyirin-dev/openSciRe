...
openSciRe takes security and trust seriously. If you discover a security vulnerability, **please do
not file a public issue**.

### Disclosure Process

1. **Report via email**: `security@openscire.dev`
   - Encrypt sensitive details using our PGP key (key ID: TBD — will be published when
     infrastructure is established).
1. **Acknowledgment**: We will acknowledge receipt within **72 hours**.
1. **Fix target**: We aim to release a fix within **14 days** of confirmation.
1. **Disclosure**: We will coordinate disclosure with you after a fix is released.

## Scope

In-scope:

- Code execution vulnerabilities in `openscire-core`, `openscire-sandbox-core`, and official
  packages
...
We ask that researchers follow coordinated disclosure practices. Please give us a reasonable window
to address issues before publication.
